...
document.addEventListener("DOMContentLoaded", async () => { 
  ...
  const defaultDateMiddle = new Date((new Date(selectedDateRange.slice(0,10)).getTime() + new Date(selectedDateRange.slice(21,31)).getTime())/2).toISOString().slice(0,10)
  // search date range should from 6 months before the middle date to 6 months after the middle date
  const startDate = new Date((new Date(defaultDateMiddle).getTime() - 6*30*24*60*60*1000))
  const endDate = new Date((new Date(defaultDateMiddle).getTime() + 6*30*24*60*60*1000))
  ...
  var dateSelection = flatpickr(dateRangeInput, {
    ...
    onClose: function (selectedDates, dateStr, instance) {
      ...
      // get the middle date of the selected date range
      const defaultDateMiddle = new Date((new Date(selectedDateRange.slice(0,10)).getTime() + new Date(selectedDateRange.slice(21,31)).getTime())/2).toISOString().slice(0,10)
      // search date range should from 6 months before the middle date to 6 months after the middle date
      const startDate = new Date((new Date(defaultDateMiddle).getTime() - 6*30*24*60*60*1000))
      const endDate = new Date((new Date(defaultDateMiddle).getTime() + 6*30*24*60*60*1000))
  ...
  async function fetchDataOverFeature(layer, dropDownContainer) {
    ...
    async function createImagePreviewBox(feature) {
      ...
      const destinationProjection = projObj.proj4; // "EPSG:3857" //
      ...
      // var previewHeight = bottom - top;
      // var previewWidth = right - left;
      // scale = previewHeight / 256;
      // previewHeight = 256;
      // previewWidth = previewWidth * scale;
      

      // var test = `${titiler_endpoint}/stac/crop/${bbox[0]},${bbox[1]},${bbox[2]},${bbox[3]}/${previewWidth}x${previewHeight}.png?url=${paras.url}&expression=${encodeURIComponent(paras.expression)}&rescale=${paras.rescale}&colormap_name=${paras.colormap_name}&minzoom=${paras.minzoom}&maxzoom=${paras.maxzoom}&asset_as_band=True`
      // console.log(test)
      // img.src = test
    ...
    try {
      ...
      if (response.ok) {
        ...
        // get features with the MGRS tile ID from properties['sentinel:MGRS tile'] equal to the MGRS tile ID of first feature
        features = features.filter(feature => feature.properties['sentinel:MGRS tile'] === features[0].properties['sentinel:MGRS tile'])
              ...
              fetch(httpsURL)
                ...
                .then(data => {
                  ...
                  function addedGeoTiffLayer() {
                    ...
                    // var titiler_endpoint = "https://192.171.169.9/";
                    // titiler url
                    var titilerURL = `${titiler_endpoint}/cog/tiles/WebMercatorQuad/{z}/{x}/{y}.png?url=${url_to_geotiff_file}&bidx=1&bidx=2&bidx=3&rescale=0,128`;
                    // check the added geotiff layers having the same url to the tiltler url
                    const added_array = addedGeoTiffLayers.map((geoTiffLayer) => {
                      return geoTiffLayer._url === titilerURL;
                     })
                    // if added array contains true, then return true
                    if (added_array.includes(true)) {
                      return;
                    }
                    ...
                    // // const titiler_endpoint = "https://titiler.xyz";
                    // const stac_item = `https://gws-access.jasmin.ac.uk/public/nceo_ard/NCEO_ARD_STAC_API/UK-sentinel-2/${data.id}.json`;
                    // const paras = {
                    //     url: stac_item,
                    //     expression: "(B8A-B04)/(B8A+B04)",  // NDVI
                    //     rescale: "0,1",
                    ...
                    // var url = `${titiler_endpoint}/stac/tiles/WebMercatorQuad/{z}/{x}/{y}.png?url=${paras.url}&expression=${encodeURIComponent(paras.expression)}&rescale=${paras.rescale}&colormap_name=${paras.colormap_name}&minzoom=${paras.minzoom}&maxzoom=${paras.maxzoom}&asset_as_band=True`
  ...
  // fetch(`${titiler_endpoint}/stac/tilejson.json?${params.toString()}`)
  //     .then(response => response.json())
  ...
  // const titiler_endpoint = "https://titiler.xyz";
  // const stac_item = `https://gws-access.jasmin.ac.uk/public/nceo_ard/NCEO_ARD_STAC_API/UK-sentinel-2/S2B_31UCV_20230625T110619.json`;
  // const paras = {
  //     url: stac_item,
  //     expression: "(B07_b1-B05_b1)/(B07_b1+B05_b1)",  // NDVI
  //     rescale: "0,1",
  //     minzoom: 11,
  //     maxzoom: 18,
  ...
  // // https://titiler.xyz/stac/tiles/WebMercatorQuad/7/63/42@1x?url=https%3A%2F%2Fgws-access.jasmin.ac.uk%2Fpublic%2Fnceo_ard%2FNCEO_ARD_STAC_API%2FUK-sentinel-2%2FS2B_31UCV_20230625T110619.json&expression=%28B07_b1-B05_b1%29%2F%28B07_b1%2BB05_b1%29&rescale=0%2C1&colormap_name=viridis

  // // var titilerURL = `${titiler_endpoint}/cog/tiles/WebMercatorQuad/{z}/{x}/{y}.png?url=${url_to_geotiff_file}&bidx=1&bidx=2&bidx=3&rescale=0,128`;

  // var url = `${titiler_endpoint}/stac/tiles/WebMercatorQuad//{z}/{x}/{y}.png?url=${paras.url}&expression=${paras.expression}&rescale=${paras.rescale}&colormap_name=${paras.colormap_name}&minzoom=${paras.minzoom}&maxzoom=${paras.maxzoom}`
  // url = encodeURIComponent(url)
  // L.tileLayer(url, {
  //   maxZoom: 15,
  //   minZoom: 5,
  ...
  // const titiler_endpoint = "https://titiler.xyz";
  // const stac_item = 'https://gws-access.jasmin.ac.uk/public/nceo_ard/NCEO_ARD_STAC_API/UK-sentinel-2/S2A_30UUA_20180927T112111.json';
  ...
  //     colormap_name: "viridis",
  // };
  // var url = `.png?url=${paras.url}&expression=${paras.expression}&rescale=${paras.rescale}&colormap_name=${paras.colormap_name}&minzoom=${paras.minzoom}&maxzoom=${paras.maxzoom}`
  // url = encodeURIComponent(url)
  // console.log(url)
  // fetch(`${titiler_endpoint}/stac/tilejson.json?${params.toString()}`)
  //     .then(response => response.json())
  //     .then(data => console.log(decodeURI(data.tiles[0])))
  //     .catch(error => console.error('Error:', error));
