...
const testApiConnection = async (apiUrl) => {
  try {
    ...
    console.error(error);
    alert(`A connection error occurred with ${apiUrl}. Please trust the certificate and try again.`);
...
document.addEventListener("DOMContentLoaded", async () => { 
  let apiUrl = "https://192.171.169.103/search"; 
  let titilerUrl = "https://192.171.169.9"

  await testApiConnection(apiUrl);
  await testApiConnection(titilerUrl);
